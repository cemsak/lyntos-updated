...
export function RiskReviewList({ items, onItemClick, selectedId, keyboardNav = true }: RiskReviewListProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-200 bg-white">
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-sm border border-slate-200 rounded-lg bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
            ...
            {(Object.entries(RISK_LEVEL_CONFIG) as [RiskLevel, typeof RISK_LEVEL_CONFIG.kritik][]).map(([level, cfg]) => {
              ...
              return (
                <button key={level} onClick={() => toggleRiskFilter(level)}
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${active ? `${cfg.bgColor} ${cfg.color} ring-2 ring-offset-1 ring-current` : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
          ...
          <div className="flex items-center gap-2">
            {(['skor', 'tarih'] as const).map(by => (
              <button key={by} onClick={() => toggleSort(by)}
                className={`px-2 py-1 text-xs rounded ${filters.sortBy === by ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-100'}`}>
      ...
      {/* Keyboard hint */}
      {keyboardNav && (
        <div className="p-2 text-center text-xs text-slate-400 border-t border-slate-100 bg-slate-50">
          <kbd className="px-1.5 py-0.5 bg-slate-200 rounded text-[10px]">J</kbd>/<kbd className="px-1.5 py-0.5 bg-slate-200 rounded text-[10px]">K</kbd> gezin, <kbd className="px-1.5 py-0.5 bg-slate-200 rounded text-[10px]">Enter</kbd> aç
        </div>
      )}
    </div>
  );
