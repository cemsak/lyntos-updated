/**
 * LYNTOS V2 - E-Defter Proxy Route
 *
 * Next.js API route that proxies requests to the Python backend.
 * Handles: /api/v2/edefter/rapor, /api/v2/edefter/durum, etc.
 */
...
async function proxy(req: NextRequest, pathParts: string[]) {
  ...
  // Build target URL: /api/v2/edefter/{path parts}
  const target = new URL(`/api/v2/edefter/${pathParts.join("/")}`, apiBase);
