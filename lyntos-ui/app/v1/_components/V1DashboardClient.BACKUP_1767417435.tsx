...
import CorporateTaxPanel from "./CorporateTaxPanel";
...
export default function V1DashboardClient(props: { contract?: PortfolioContract | null; ctx: Ctx }) {
  ...
  // ALL useMemo hooks MUST be before early return (React rules of hooks)
...
{(() => {
  ...
  <a href={`/v1?smmm=${encodeURIComponent(props.ctx.smmm)}&client=${encodeURIComponent(props.ctx.client)}&period=${encodeURIComponent(props.ctx.period)}#axis-d`}>Axis-D (link)</a>
        ...
        {/* BEGIN CORPORATE_TAX_PANEL */}
        <CorporateTaxPanel
          smmmId={props.ctx.smmm}
          clientId={props.ctx.client}
          period={props.ctx.period}
        />
        {/* END CORPORATE_TAX_PANEL */}
