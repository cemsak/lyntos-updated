...
def compute_edefter_metrics(edefter_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    E-defter metrikleri (VDK-grade delil desteği):
    - teknik sinyaller (KDV/Fx/enflasyon satır oranı)
    - tarih kapsaması (date_range) + dated/undated sayıları
    - satış delili (600/601/602) + iskontolar (610/611/612) + net satış (ay bazında)
    - inceleme paketi için örnek satırlar (ay bazında küçük sample)
    """
    ...
    SAMPLE_LIMIT = 10
    samples_sales_by_month = {}
    samples_kdv_by_month = {}

    def add_sample(bucket: dict, month: str, row: dict):
        arr = bucket.setdefault(month, [])
        if len(arr) >= SAMPLE_LIMIT:
            return
        arr.append({
            "tarih": row.get("tarih"),
            "yevmiye_no": row.get("yevmiye_no") or row.get("fis_no") or row.get("fiş_no"),
            "belge_no": row.get("belge_no") or row.get("doc_no") or row.get("fatura_no"),
            "hesap_kodu": row.get("hesap_kodu"),
            "hesap_adi": row.get("hesap_adi"),
            "borc": row.get("borc"),
            "alacak": row.get("alacak"),
            "aciklama": row.get("aciklama") or row.get("detay"),
    ...
    for row in (edefter_list or []):
        ...
        text = (name + " " + aciklama + " " + detay).strip()

        d = parse_date(row.get("tarih"))
        if d:
            dated_lines += 1
        ...
        if code.startswith(("190", "191", "360")) or "KDV" in text:
            kdv_lines += 1
            if month:
                add_sample(samples_kdv_by_month, month, row)
        ...
        # satış delili
        if month:
            borc = to_float(row.get("borc"))
            alacak = to_float(row.get("alacak"))

            if any(code.startswith(p) for p in sales_accounts):
                amt = max(0.0, float(alacak) - float(borc))
                per_month_sales[month] = per_month_sales.get(month, 0.0) + amt
                add_sample(samples_sales_by_month, month, row)

            if any(code.startswith(p) for p in discount_accounts):
                disc = max(0.0, float(borc) - float(alacak))
                per_month_discounts[month] = per_month_discounts.get(month, 0.0) + disc

    # net satış
    months = sorted(set(per_month_sales.keys()) | set(per_month_discounts.keys()))
    for m in months:
        s = float(per_month_sales.get(m, 0.0))
        d = float(per_month_discounts.get(m, 0.0))
        per_month_net[m] = s - d

    totals = {
        "sales_total_600_601_602": sum(float(v) for v in per_month_sales.values()),
        "discount_total_610_611_612": sum(float(v) for v in per_month_discounts.values()),
        "net_sales_total": sum(float(v) for v in per_month_net.values()),
    }
    ...
    return {
        ...
        "date_range": {"min": (dmin.isoformat() if dmin else None), "max": (dmax.isoformat() if dmax else None)},
        "counts": {
            "dated_lines": dated_lines,
            "undated_lines": undated_lines,
            "dated_ratio": (dated_lines / total_lines) if total_lines else None,
        },
        "sales_evidence": {
            ...
            "per_month_net_sales": per_month_net,
            "samples_sales_by_month": samples_sales_by_month,
            "samples_kdv_by_month": samples_kdv_by_month,
            "totals": totals,
        },
    }

    def _safe_float(x):
        """Locale + numeric-type safe float parser.
        TR: 1.234.567,89 -> 1234567.89
        EN: 1,234,567.89 -> 1234567.89
        EN: 1234567.89   -> 1234567.89
        """
        if x is None:
            return 0.0
        ...
        if ',' in s and '.' in s:
            ...
            # only dot or none; if multiple dots, keep last as decimal
            if s.count('.') > 1:
...
def run_risk_model_v1(
    ...
    try:
        ...
        # Mark for debugging
        _r = metrics.get('rules')
        if isinstance(_r, dict):
            _r['enriched'] = True
            _r.pop('enrich_error', None)
    except Exception as e:
        _r = metrics.get('rules')
        if isinstance(_r, dict):
            _r['enriched'] = False
            _r['enrich_error'] = f"{e.__class__.__name__}: {e}"
