...
class RiskAnalyzer:
    ...
    def analyze(self, *, mizan_deep: Dict[str, Any], beyan_metrics: Dict[str, Any], bank_metrics: Dict[str, Any],
        ...
        # --- FIX_R501_SAFE_ROLLUP_TOL_V3 ---
        # R-501 için beyanname↔mizan toleransı, bank mutabakat toleransından bağımsız olmalıdır.
        tol_beyan = float(th.get("tolerance_beyan_mizan", 0.02))

        # Mizan satış toplamında parent+child double-count engeli:
        # parent (600/601/602) varsa parent kullan; yoksa leaf (en alt) hesapları topla.
        def _family_total(values: dict, root: str):
            items = {k: float(v or 0.0) for k, v in (values or {}).items()
                     if (k == root) or k.startswith(root + ".")}
            if not items:
                return 0.0, []
            if root in items and abs(items.get(root, 0.0)) > 0.0:
                return float(items[root]), [root]
            keys = list(items.keys())
            leaves = []
            for c in keys:
                pref = c + "."
                if not any((o != c and o.startswith(pref)) for o in keys):
                    leaves.append(c)
            if not leaves:
                leaves = keys
            return float(sum(items[c] for c in leaves)), sorted(leaves)

        balances = (mizan_deep or {}).get("balances") or {}
        sales_vals = {k: max(float(v or 0.0), 0.0) for k, v in balances.items()
                      if k.startswith(("600", "601", "602"))}

        gross_sales_raw = gross_sales
        sales_used = {}
        gross_sales_rollup = 0.0
        for root in ("600", "601", "602"):
            tot, used = _family_total(sales_vals, root)
            if abs(tot) > 0.0:
                gross_sales_rollup += tot
                sales_used[root] = {"total": tot, "used_codes": used}

        # balances tabanlı rollup bulunduysa onu esas al
        if gross_sales_rollup > 0:
            gross_sales = gross_sales_rollup
        ...
        if kdv_matrah is not None and gross_sales > 0:
            ...
            if rel > tol_beyan:
                self._add(
                    ...
                    threshold={"rel_tolerance": tol_beyan, "severity_high_rel": 0.10},
