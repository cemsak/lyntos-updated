...
def parse_banka_csv(path: Path) -> List[Dict[str, Any]]:
    ...
    for row in dict_rows:
        ...
        # --- 2a) Akbank vb. format: 'Borç/Alacak' kolonu A/B + 'Tutar' ---
        ba_flag = _try_get(row, ["Borç/Alacak","Borc/Alacak","Borç / Alacak","Borc / Alacak","B/A","B-A","B A"])
        if ba_flag:
            ba_flag = ba_flag.strip().upper()

        if ba_flag in ("A","B"):
            tutar_str = _try_get(
                row,
                ["Tutar","Tutar(TL)","İşlem Tutarı","Islem Tutarı","Islem Tutari","Hareket Tutarı","Hareket Tutari","TUTAR","TUTARI"],
            )
            amount = abs(_parse_tr_number(tutar_str))
            if ba_flag == "B":  # Borç
                borc = amount
                alacak = 0.0
            else:              # Alacak
            ...
            if borc_str or alacak_str:
                ...
                # --- 3) Tek 'Tutar' kolonu varsa, işarete göre borç/alacak ayır ---
                tutar_str = _try_get(
                    row,
                    ["Tutar","Tutar(TL)","İşlem Tutarı","Islem Tutarı","Islem Tutari","Hareket Tutarı","Hareket Tutari","TUTAR","TUTARI"],
                )
...
def parse_banka_for_client(
    ...
    """
    Banka CSV'lerini toplar ve parse eder.

    Yapı (önerilen ve desteklenen):
      data/banka/<SMMM>/<CLIENT>/<PERIOD>/_raw/*.csv   (asıl ham CSV'ler)
      data/banka/<SMMM>/<CLIENT>/<PERIOD>/_archive/... (arşiv)

    Geriye dönük destek:
      data/banka/<SMMM>/<CLIENT>/<PERIOD>/*.csv
    """
    folder = base_dir / "banka" / smmm_id / entity_id / period
    if not folder.exists():
        raise FileNotFoundError(f"Banka klasörü yok: {folder}")

    # Öncelik: _raw varsa orayı oku; yoksa period kökünü tara.
    scan_root = (folder / "_raw") if (folder / "_raw").exists() else folder

    all_records: List[Dict[str, Any]] = []
    for csv_path in sorted(scan_root.rglob("*.csv")):
        # _archive altı varsa atla; _raw kalsın
